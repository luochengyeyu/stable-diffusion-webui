import inspect
# Pydantic是一个用于数据验证和设置管理的Python库。它旨在填补Python标准库在数据验证方面的空白。
# BaseModel是所有模型的基类，它提供了验证和序列化数据的功能。
# Field是一个类，它用于定义模型中的字段，并提供了对字段的额外配置和验证。
# create_model是一个函数，用于根据给定的参数动态创建模型。
...
class PydanticModelGenerator:
    ...
    def __init__(
        ...
        """
            ...
            :param class_instance: 类实例
            :param additional_fields: 附加字段
        """
        ...
        def field_type_generator(k, v):
            ...
            if field_type == 'Image':
                ...
                # 图像通过 API 作为 base64 字符串发送
                field_type = 'str'
            # Optional是一个类型提示函数，用于表示一个值可以是任何类型，或者为空（None）。
            return Optional[field_type]

        def merge_class_params(class_):
            """
            这个函数接受一个类作为参数，然后返回这个类及其所有父类中的所有构造函数的参数。
            """
            # inspect.getmro(class_) 返回一个包含 class_ 及其所有父类（包括 object）的元组。
            # 使用 filter 和一个 lambda 函数来只保留不是 object 的类。
            all_classes = list(filter(lambda x: x is not object, inspect.getmro(class_)))
            parameters = {}
            # 合并所有父类的参数到一个字典中
            for classes in all_classes:
                # 使用 inspect.signature(classes.__init__) 来获取当前类的构造函数的签名，并获取其参数。
        ...
        self._model_def = [
            ModelDef(
                # 将k处理为带下划线的小写形式并赋值给field
                field=underscore(k),
                field_alias=k,
                field_type=field_type_generator(k, v),
                # 如果 v 的默认值是一个属性，则将字段的值初始化为 None，否则将使用变 v 的默认值。
                field_value=None if isinstance(v.default, property) else v.default
            )
            # 遍历_class_data字典得到参数名k及参数值v，用于生成ModelDef列表
            # (除去参数名在API_NOT_ALLOWED列表中的)
            for (k, v) in self._class_data.items() if k not in API_NOT_ALLOWED
        ]

        # 遍历附件参数，将之添加到_model_def列表
    ...
    def generate_model(self):
        ...
        """
        fields = {
            # 这是一个字典推导式，它遍历self._model_def中的每个元素，并将每个元素的field属性作为键，对应的值是一个元组，
            # 元组包含 field_type 和一个 Field 对象
